...
class WOLightSource(LightSource, LightSourceDecorator):
    ...
    def get_wavefront(self):
        ...
        if self._kind_of_wave == 0:  # plane
            ...
            #                                            mode_x=self.mode_index, shift=self.gaussian_shift,
            #                                            beta=self.gaussian_beta)
    ...
    def to_python_code(self, do_plot=True):
        ...
        if self._dimension == 1:
            ...
            if self._initialize_from == 0:
                ...
                txt += "\ninput_wavefront = GenericWavefront2D.initialize_wavefront_from_steps(x_start=%g,x_step=%g,y_start=%g,y_step=%g,number_of_points=(%d,%d))" % \
        ...
        if self._units == 0:
            txt += "\ninput_wavefront.set_photon_energy(%g)"%(self._energy)
        else:
            txt += "\ninput_wavefront.set_wavelength(%g)"%(self._wavelength)
        ...
        if self._kind_of_wave == 0: #plane
            ...
            if self._dimension == 1:
                ...
                txt += "\ninput_wavefront.set_gaussian(sigma_x=%f, sigma_y=%f, amplitude=%f)"%\
            ...
            if self._dimension == 1:
                ...
                txt += "\ninput_wavefront.set_gaussian_hermite_mode(sigma_x=%g,sigma_y=%g,amplitude=%g,nx=%d,ny=%d,betax=%g,betay=%g)"%\
        ...
        if do_plot:
            if self._dimension == 1:
                ...
                txt += "\nplot_image(input_wavefront.get_intensity(),input_wavefront.get_coordinate_x(),input_wavefront.get_coordinate_y(),aspect='auto')"
